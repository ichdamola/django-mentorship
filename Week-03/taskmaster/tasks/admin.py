...
class CategoryAdmin(admin.ModelAdmin):
    ...
    def colored_badge(self, obj):
        return format_html('<span style="background-color: {}; padding: 3px 10px; '
                           'bborder-radius: 3px; color: white;">{}</span>', 
                           obj.color, obj.name)
...
class TaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ('Status', {'fields': ['status', 'priority', 'category']}),
        ('Dates', {'fields': ['due_date', 'created_at', 'updated_at', 'completed_at'], 'classes': ['collapse']})
    ]
